...
import { useLiveOgnData } from '@/hooks/useLiveOgnData';
import { useAcarsData } from '@/hooks/useAcarsData';
...
export default function Home() {
  ...
  const setMapStyle = useCallback((style: 'dark' | 'satellite') => {
    setFilters((prev) => {
  ...
  const setSeismicRange = useCallback((range: 'live' | '24h') => {
    setFilters((prev) => {
      const next = { ...prev, seismicRange: range };
      try {
        localStorage.setItem('defcon_filters', JSON.stringify(next));
      } catch (e) {
        console.warn('Could not save filters to localStorage', e);
  ...
  const setThermalRange = useCallback((range: 'live' | '24h') => {
    setFilters((prev) => {
      const next = { ...prev, thermalRange: range };
      try {
        localStorage.setItem('defcon_filters', JSON.stringify(next));
      } catch (e) {
        console.warn('Could not save filters to localStorage', e);
  ...
  // ─── Data hooks ─────────────────────────────────────────────
  const { aircraft, connectionStatus: adsbStatus, providerName: adsbProvider, rawCount, forceRefresh, version: acVersion } = useLiveAircraftData();
  const apiKey = process.env.NEXT_PUBLIC_AISSTREAM_KEY;
  const { ships, connectionStatus: aisStatus, apiKeyMissing: aisKeyMissing, version: shipVersion } = useLiveShipData(apiKey, filters);
  const { ognVehicles, ognConnectionStatus } = useLiveOgnData();

  const airframesKey = process.env.NEXT_PUBLIC_AIRFRAMES_KEY;
  const { messages: acarsMessages, connectionStatus: acarsConnectionStatus } = useAcarsData(airframesKey);

  const { earthquakes, status: usgsStatus } = useUsgsData(filters.showSeismic, filters.seismicRange);
  const { fires, status: firmsStatus } = useFirmsData(filters.showThermal, filters.thermalRange);
  const { radiationData, statuses: radiationStatuses } = useRadiationData(filters.showRadiation);
  ...
  const handleSelect = useCallback((vehicle: SelectedVehicle) => {
    ...
    try {
      if (vehicle) {
        const id = vehicle.kind === 'aircraft' ? vehicle.data.hex :
          vehicle.kind === 'ship' ? String(vehicle.data.mmsi) :
            vehicle.data.id;
      ...
      console.warn('Could not save selection to localStorage (Quota Exceeded?)', e);
      // Attempt to clean up old keys from other projects that might be hogging space on localhost
      try {
        localStorage.removeItem('echosky_ships');
        localStorage.removeItem('echosky_filters');
      } catch { /* ignore */ }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (kind === 'aircraft') {
        ...
        const ogn = ognVehicles.find(v => v.id === id);
        if (ogn) { setSelected({ kind: 'ogn', data: ogn }); restoredRef.current = true; }
      }
    } catch { restoredRef.current = true; }
  }, [acVersion, shipVersion, ognVehicles]);
  ...
  aircraft.forEach((ac) => {
    ...
    if (ac.isGround) counts.ground++;
    if (ac.isMlat) counts.mlat++;
    if (ac.isUat) counts.uat++;
  });
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden">
      ...
      <FiltersPanel
        ...
        onSetSeismicRange={setSeismicRange}
        onSetThermalRange={setThermalRange}
        onSetMapStyle={setMapStyle}
        counts={counts}
        darkVehicleCount={darkVehicleCount}
        adsbStatus={adsbStatus}
        adsbProvider={adsbProvider}
        aisStatus={aisStatus}
        aisKeyMissing={aisKeyMissing}
        ognConnectionStatus={ognConnectionStatus}
        acarsStatus={acarsConnectionStatus}
        usgsStatus={usgsStatus}
        firmsStatus={firmsStatus}
        radiationStatuses={radiationStatuses}
        ...
        radiationCount={radiationData.length}
      />
      ...
      <div className="absolute top-[48px] left-0 right-0 bottom-0">
        <MapView
          ...
          dataVersion={acVersion + shipVersion + ognVehicles.length + earthquakes.length + fires.length + radiationData.length}
        />
      </div>
